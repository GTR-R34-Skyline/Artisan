...
const VoiceProfileCapture: React.FC<VoiceProfileCaptureProps> = ({ onSubmit }) => {
  ...
  const submitTranscript = useCallback(async (
    payload: { transcript?: string; audioBlob?: Blob; durationSec?: number },
    language: SupportedLanguageCode,
  ) => {
    ...
    try {
      ...
      const response = await ProfileConversationService.sendTurn({
        ...request,
      });
  ...
  const stopListeningAndSubmit = useCallback(() => {
    ...
    recorder.onstop = () => {
      ...
      if (selectedLanguage) void submitTranscript({ audioBlob, durationSec }, selectedLanguage);
    };
    recorder.stop();
