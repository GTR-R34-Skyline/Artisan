/**
 * Deterministic seller-profile field extraction (Edge Function copy).
 * Keep in sync with src/services/profileFieldExtraction.ts
 */
...
export const extractSellerProfileFields = (transcript: string): ExtractedSellerProfileFields => {
  ...
  const merged = extractNaturalLanguage(text, labeled);
